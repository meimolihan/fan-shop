...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-shop 非凡商店容器管理平台 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-shop"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8000"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-shop"
# ====================================================================================
...
services:
   fan-shop:
      image: mobufan/fan-shop:latest
      container_name: fan-shop
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据说明：数据/仓库/脚本/备份/日志/镜像均持久化挂载于 backend/ 下，卸载不会丢失"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
