...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-reubah 通用文件转换与图像处理 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-reubah"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="8081"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-reubah"
# ====================================================================================
...
services:
   fan-reubah:
      image: mobufan/fan-reubah:latest
      container_name: fan-reubah
      network_mode: bridge
      restart: unless-stopped
      environment:
         - TZ=Asia/Shanghai
      # 无状态服务：处理完即交付，容器内不持久化数据
      # 如需持久化临时/缓存目录，可取消注释（确保默认目录已 chown）
      # volumes:
      #    - ./data:/tmp
      ports:
         - ${HOST_PORT}:8081
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据说明：无状态交付，容器重建后不保留处理记录；SVG 矢量化依赖镜像内置 vtracer"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
