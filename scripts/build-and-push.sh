...
# 直接 --limit 1 取最新会取到上一次（如 v2.1.8）的陈旧记录。
...
[[ "${TAG}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || error "TAG 格式应为 vX.Y.Z，例如 v2.1.7（当前: ${TAG}）"
...
info "写入发版备注 RELEASE_NOTES.md"
{
  ...
  printf '```bash\n'
  printf 'docker pull mobufan/fan-shop:latest\n'
  printf '```\n'
  printf '```bash\n'
  printf 'docker pull mobufan/fan-shop:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/fan-shop:latest\n'
  printf '```\n'
  printf '```bash\n'
  printf 'docker pull ghcr.io/meimolihan/fan-shop:%s\n' "${TAG}"
  printf '```\n'
  printf '\n'
  printf '## 源码安装\n'
  printf '```bash\n'
  printf 'bash <(curl -fsSL https://raw.githubusercontent.com/meimolihan/fan-shop/main/scripts/install.sh)\n'
  printf '```\n'
  printf '\n'
  printf '## 源码卸载（保留数据）\n'
  printf '```bash\n'
  printf 'bash <(curl -fsSL https://raw.githubusercontent.com/meimolihan/fan-shop/main/scripts/uninstall.sh)\n'
  printf '```\n'
  printf '\n'
  printf '## 源码卸载（连数据一并删除）\n'
  printf '```bash\n'
  printf 'bash <(curl -fsSL https://raw.githubusercontent.com/meimolihan/fan-shop/main/scripts/uninstall.sh) --purge\n'
  printf '```\n'
} > RELEASE_NOTES.md
