...
# ====================== 【可自定义配置区】 在这里修改所有默认参数 ======================
# 项目标题
DEFAULT_TITLE="fan-video 本地视频媒体服务器 一键部署"

# 部署目录（不传参时的默认路径）
DEFAULT_COMPOSE_DIR="/vol1/1000/compose/fan-video"

# 默认访问端口（不传参时使用）
DEFAULT_PORT="9060"

# 默认容器名称（可自定义）
DEFAULT_CONTAINER_NAME="fan-video"

# 媒体库挂载目录（只读挂载到容器 /media，供扫描/首帧使用）
DEFAULT_MEDIA_DIR="/vol2/1000/mydisk/Video"
# ====================================================================================
...
deploy_app() {
    ...
    # 检测硬件加速设备 /dev/dri（Intel/AMD 核显），存在则挂载，否则跳过以免 compose 启动失败
    local DRI_DEVICES=""
    if [ -e "/dev/dri" ]; then
        ...
        log_warn "未检测到 /dev/dri，跳过硬件加速挂载（软件解码兜底）"
    fi
...
services:
   fan-video:
      image: mobufan/fan-video:latest
      ...
      volumes:
         - ./data:/data
         - ./cache:/cache
         # 媒体库只读挂载，用于扫描与首帧提取（如需修改请编辑本行或上方 DEFAULT_MEDIA_DIR）
...
EOF
    ...
    log_info "部署目录：${gl_huang}${COMPOSE_DIR}${gl_bai}"
    log_info "数据目录：${gl_huang}${COMPOSE_DIR}/data${gl_bai} (数据库) / ${gl_huang}${COMPOSE_DIR}/cache${gl_bai} (缓存)"
    log_info "首次登录：账号 ${gl_lv}admin${gl_bai}，默认密码见容器启动日志：${gl_huang}docker logs fan-video 2>&1 | grep -i password${gl_bai}"
    echo -e "${gl_bufan}————————————————————————————————————————————————${gl_bai}"
    break_end
}
